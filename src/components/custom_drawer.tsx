  ...
  return (
    <>
      ...
      <button
        ...
        border border-gray-200 h-[43px] px-2 rounded-md shadow flex flex-row justify-between items-center gap-3
        transition transition-all transform duration-200 hover:shadow-lg`}
      >
        <TbMenu3 className="w-5 h-5" />
        {profile ? (
          <div className={`${isScrolled ? "bg-black": "bg-transparent"} text-white flex items-center justify-center w-7 h-7 rounded-full font-medium text-sm`}>
            {initial}
          </div>
        ) : (
