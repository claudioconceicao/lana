...
export default function LoginAndSecurity(){
  ...
  const linkedAccounts: { name: string; provider: Provider; status: string }[] =
    [
  ...
  function getProvider(provider: string) {
    ...
    const updateEmail = async () => {
      if (!email) return alert("O email não pode estar vazio.");
    ...
    const updatePassword = async () => {
      ...
      if (!newPassword) return alert("A nova senha não pode estar vazia.");
    ...
    const toggleTwoFA = () => {
      // Placeholder for real Supabase 2FA logic
      setTwoFA(!twoFA);
    };
    ...
    const deleteAccount = async () => {
      ...
      if (userError || !userData?.user) {
        return alert("Erro ao obter informações do usuário.");
      }

      // Log out user instead of calling deleteUser (requires admin privileges)
      await supabase.auth.signOut();
      alert("Conta marcada para exclusão. Você foi desconectado.");
    };
    ...
    return (
      <div className="w-full max-w-3xl mx-auto space-y-12 p-6">
        ...
        <section className="space-y-3">
          ...
          <p className="text-gray-600">
            Se você deseja excluir permanentemente sua conta, esta ação não
            poderá ser desfeita.
          </p>
          <button
            type="button"
